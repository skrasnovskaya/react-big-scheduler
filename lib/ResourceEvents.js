...
var ResourceEvents = (_temp = _class = function (_Component) {
    ...
    function ResourceEvents(props) {
        ...
        var _props$schedulerData = props.schedulerData,
            disabledIndex = _props$schedulerData.disabledIndex,
            config = _props$schedulerData.config,
            headers = _props$schedulerData.headers;


        _this.disabledIndex = disabledIndex;
        _this.isRtl = config.isRtl;

        if (_this.isRtl) {
            _this.disabledIndex = headers.length - disabledIndex;
        }
                ...
                if (headerItem.count > 0 || headerItem.summary != undefined) {
                    ...
                    headerItem.events.forEach(function (evt, idx) {
                        if (idx < renderEventsMaxIndex && evt !== undefined && evt.render) {
                            ...
                            var offset = 0;
                            if (isStart && cellUnit !== _index.CellUnits.Hour) {
                                offset = cellWidth / 2;
                            }
                            var _left = index * cellWidth + offset;
                            offset = 0;
                            if (isEnd && cellUnit !== _index.CellUnits.Hour) {
                                offset = evt.span > 1 ? isStart ? cellWidth : cellWidth / 2 : cellWidth / 2;
                            }

                            var _width = evt.span * cellWidth - (index > 0 ? 5 : 6) > 0 ? evt.span * cellWidth - offset : 0;
    ...
    this.initDrag = function (ev) {
        ...
        if ((ev.srcElement || ev.target) !== _this3.eventContainer) return;

        var schedulerData = _this3.props.schedulerData;

        var cellWidth = schedulerData.getContentCellWidth();
        var pos = (0, _Util.getPos)(_this3.eventContainer);
        ...
        if (_this3.isRtl) {
            if (rightIndex > _this3.disabledIndex) return;
        } else {
            if (leftIndex < _this3.disabledIndex) return;
        }
        ...
        document.documentElement.addEventListener('mousemove', _this3.doDrag, false);
        document.documentElement.addEventListener('mouseup', _this3.stopDrag, false);
    };
    ...
    this.stopDrag = function (ev) {
        ...
        var _props2 = _this3.props,
            ...
            newEvent = _props2.newEvent,
            resourceEvents = _props2.resourceEvents;
        var headers = schedulerData.headers,
        ...
        var _state2 = _this3.state,
            leftIndex = _state2.leftIndex,
            rightIndex = _state2.rightIndex;

        document.documentElement.removeEventListener('mousemove', _this3.doDrag, false);
        document.documentElement.removeEventListener('mouseup', _this3.stopDrag, false);

        var startHeaders = config.isRtl ? [].concat(_toConsumableArray(headers)).reverse() : headers;
        var endHeaders = config.isRtl ? [].concat(_toConsumableArray(resourceEvents.headerItems)).reverse() : resourceEvents.headerItems;

        var startTime = startHeaders[leftIndex].time;
        var endTime = endHeaders[rightIndex - 1].end;
        if (cellUnit !== _index.CellUnits.Hour) {
            endTime = localeMoment(endHeaders[rightIndex - 1].start);
            endTime = config.isRtl ? endTime.format(_index.DATETIME_FORMAT) : endTime.hour(23).minute(59).second(59).format(_index.DATETIME_FORMAT);
        }

        var noTimeDayView = config.nonTimeDayView && cellUnit === _index.CellUnits.Hour; // Fix for custom day view (two half)
        ...
        if (hasConflict) {
            var conflictOccurred = _this3.props.conflictOccurred;
    ...
    this.onAddMoreClick = function (headerItem) {
        var _props3 = _this3.props,
            onSetAddMoreState = _props3.onSetAddMoreState,
            resourceEvents = _props3.resourceEvents,
    ...
    this.eventContainerRef = function (element) {
        _this3.eventContainer = element;
    };
}, _temp);
