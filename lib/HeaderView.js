        ...
        value: function render() {
            ...
            if (cellUnit === _index.CellUnits.Hour) {
                ...
                if (!!config.nonTimeDayView) {
                    ...
                    var pList = config.nonAgendaDayCellHeaderFormat.split('|').map(function (item) {
                        return datetime.format(item);
                    })[0];

                    var headerElement = _react2.default.createElement(
                        'th',
                        { className: 'header3-text day-view no-time' },
                    ...
                    headers.forEach(function (item, index) {
                        if (index % minuteStepsInHour === 0) {
                            var _datetime = localeMoment(item.time);
                            var isCurrentTime = _datetime.isSame(new Date(), 'hour');
                            ...
                            if (typeof nonAgendaCellHeaderTemplateResolver === 'function') {
                                ...
                                var _pList = pFormattedList.map(function (item, index) {
