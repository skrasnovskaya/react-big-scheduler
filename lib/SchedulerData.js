...
var SchedulerData = function () {
    ...
    _createClass(SchedulerData, [{
        ...
        key: 'setEdgeDate',
        value: function setEdgeDate(momentDate) {
            this.edgeDate = (0, _moment2.default)(momentDate).isValid() ? (0, _moment2.default)(momentDate) : null;
            this._createHeaders();
        }
    }, {
        ...
        value: function getContentCellWidth() {
            if (this.config.dynamicCellWidth && this.customWidth) {
                var adaptiveWidth = Math.ceil(this.customWidth / this.headers.length);
                return this.viewType === _index.ViewTypes.Week ? Math.max(this.config.weekCellWidth, adaptiveWidth) : this.viewType === _index.ViewTypes.Day ? Math.max(this.config.dayCellWidth, adaptiveWidth) : this.viewType === _index.ViewTypes.Month ? Math.max(this.config.monthCellWidth, adaptiveWidth) : this.viewType === _index.ViewTypes.Year ? this.config.yearCellWidth : this.viewType === _index.ViewTypes.Quarter ? this.config.quarterCellWidth : Math.max(this.config.customCellWidth, adaptiveWidth);
            }
        ...
        value: function _createHeaders() {
            ...
            if (_moment2.default.isMoment(edgeDate)) {
                // check if edgeDate is in displayed range
                var startCopy = this.localeMoment(this.startDate);

                switch (true) {
                    case edgeDate.isSame(startCopy):
                        this.disabledIndex = 0;
                        if (this.cellUnit === _index.CellUnits.Hour && this.config.nonTimeDayView) {
                            this.disabledIndex = 1;
                        }
                        break;
                    case edgeDate.isBetween(startCopy, end):
                        for (var _i = 0; _i < headerMoments.length; _i++) {
                            if (edgeDate.isSame(headerMoments[_i])) {
                                this.disabledIndex = _i;
                                break;
                            }
                        }
                        for (var _i2 = 0; _i2 < this.disabledIndex; _i2++) {
                            headers[_i2].disabled = true;
                        }
                        break;
        ...
        value: function _createInitHeaderEvents(header) {
            ...
            var endValue = this.showAgenda ? this.viewType === _index.ViewTypes.Week ? start.add(1, 'weeks').format(_index.DATETIME_FORMAT) : this.viewType === _index.ViewTypes.Day ? start.add(1, 'days').format(_index.DATETIME_FORMAT) : this.viewType === _index.ViewTypes.Month ? start.add(1, 'months').format(_index.DATETIME_FORMAT) : this.viewType === _index.ViewTypes.Year ? start.add(1, 'years').format(_index.DATETIME_FORMAT) : this.viewType === _index.ViewTypes.Quarter ? start.add(1, 'quarters').format(_index.DATETIME_FORMAT) : this.localeMoment(this.endDate).add(1, 'days').format(_index.DATETIME_FORMAT) : this.cellUnit === _index.CellUnits.Hour ? this.config.nonTimeDayView ? start.add(12, 'hours').format(_index.DATETIME_FORMAT) : start.add(this.config.minuteStep, 'minutes').format(_index.DATETIME_FORMAT) : start.add(1, 'days').format(_index.DATETIME_FORMAT);
            return {
                time: header.time,
        ...
        value: function _createRenderData() {
            ...
            initRenderData.forEach(function (resourceEvents) {
                ...
                resourceEvents.rowHeight = _this5.config.eventItemLineHeight + 2; //rowsCount === 0 ? this.config.eventItemLineHeight + 2 : rowsCount * this.config.eventItemLineHeight + (this.config.creatable && this.config.checkConflict === false ? 20 : 2);
